...
################################################
$envFile = Join-Path $PSScriptRoot "..\..\src\Workload\.env"
if (-not (Test-Path $envFile)) {
    Write-Error "Environment file not found at $envFile. Please copy .env.template to .env and configure it."
    exit 1
}
...
################################################
$templatePath = Join-Path $PSScriptRoot "..\..\src\Workload\Manifest"
...
################################################
$nugetPath = Join-Path $PSScriptRoot "..\..\src\Workload\node_modules\nuget-bin\nuget.exe"
...
if (-not (Test-Path $nugetPath)) {
    ...
    $workloadDir = Join-Path $PSScriptRoot "..\..\src\Workload"
    try {
...
# Check if running on Windows (handles both PowerShell 5.x and 7.x)
$isWindowsOS = $IsWindows -or ($env:OS -eq "Windows_NT") -or ([System.Environment]::OSVersion.Platform -eq "Win32NT")
...
################################################
# Update workload-dev-mode.json to point to new manifest
################################################
$devModeFile = Join-Path $PSScriptRoot "..\..\build\DevGateway\workload-dev-mode.json"
if (Test-Path $devModeFile) {
    $workloadName = $envVars['WORKLOAD_NAME']
    $workloadVersion = $envVars['WORKLOAD_VERSION']
    $manifestFullPath = Join-Path (Resolve-Path $outputDir) "$workloadName.$workloadVersion.nupkg"

    # Read existing config to preserve WorkspaceGuid
    $devModeConfig = Get-Content $devModeFile | ConvertFrom-Json
    $devModeConfig.ManifestPackageFilePath = $manifestFullPath

    $devModeConfig | ConvertTo-Json | Set-Content $devModeFile
    Write-Host "Updated workload-dev-mode.json to point to $workloadName.$workloadVersion.nupkg" -ForegroundColor Green
}
